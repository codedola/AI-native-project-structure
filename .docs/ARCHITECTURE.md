...
## 5. Hệ thống hỗ trợ phát triển (Agentic Skills)

- **Cơ chế Trigger:** Sử dụng các lệnh Slash (ví dụ: `/save`, `/archive`) để tự động hóa quy trình quản lý dự án.
- **Quản lý ngữ cảnh:** 
    - `/save`: Tóm tắt tiến độ vào `FEATURES_DONE.md`.
    - `/archive`: Nén bộ nhớ, cập nhật tài liệu kiến trúc và dọn dẹp nhật ký.
